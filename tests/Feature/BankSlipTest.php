...
class BankSlipTest extends TestCase
{
    ...
    public function test_generate_bank_slip_invalid_state(): void
    {
        $data = [
            'person_name' => fake()->name(),
            'person_city' => fake()->city(),
            'person_uf' => 'XX', // Invalid state
            'person_document' => '12345678901',
            'person_address' => fake()->streetAddress(),
            'person_zipcode' => fake()->numerify('########'),
            'bill_amount' => fake()->randomFloat(2, 10, 1000),
        ...
        $this->assertArrayHasKey('errors', $jsonContent, 'Response does not contain errors key: ' . $response->getContent());
        $this->assertArrayHasKey('person_uf', $jsonContent['errors'], 'Response does not contain person_uf error: ' . $response->getContent());
    }
